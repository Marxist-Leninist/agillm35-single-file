#!/usr/bin/env python3
"""Distributed inference harness for the real AGILLM4 transformer blocks.
...
AGILLM4 Block implementation, including MoE FFNs and sublinear attention when
enabled by the checkpoint/runtime config. The coordinator keeps embeddings,
final norm, and AR head.
"""
...
def load_agillm4(path: str | Path):
    ...
    sys.modules.setdefault("agillm4_runtime", module)
...
def block_kwargs(runtime: Any, cfg: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return {
...
def encoder_kwargs(runtime: Any, args: argparse.Namespace) -> dict[str, Any]:
    return {
        "attn_backend": args.attn_backend,
        "sublinear_window": int(args.sublinear_window),
        "sublinear_stride": int(args.sublinear_stride),
        ...
        "sublinear_pooled_landmarks": bool(args.sublinear_pooled_landmarks),
        "anchor_memory": False,
        "anchor_stride": getattr(runtime, "DEFAULT_ANCHOR_STRIDE", 256),
        "anchor_max": getattr(runtime, "DEFAULT_ANCHOR_MAX", 2048),
        "anchor_position": getattr(runtime, "DEFAULT_ANCHOR_POSITION", -1),
    }
...
def tensor_from_payload(data: bytes) -> dict[str, Any]:
    ...
    if len(data) < len(WIRE_MAGIC) + 4 or not data.startswith(WIRE_MAGIC):
        raise ValueError("bad AGILLM4 inference wire payload")
    ...
    if header_len <= 0 or header_len > 1_000_000 or header_end > len(data):
        raise ValueError("bad AGILLM4 inference wire header")
...
class WorkerHandler(BaseHTTPRequestHandler):
    server_version = "AGILLM4DistributedInferWorker/1"
...
def cmd_worker(args: argparse.Namespace) -> None:
    runtime = load_agillm4(args.agillm4_path)
    sd = load_ckpt(runtime, args.ckpt)
    args.device = resolve_device(args.device)
    stage = StageModule(runtime, sd, args.start_layer, args.end_layer, args.device, args)
    del sd
    gc.collect()
...
def parse_stage_specs(args: argparse.Namespace, runtime: Any, sd: dict[str, Any]) -> list[Any]:
    ...
    for idx, spec in enumerate(specs):
        if spec.startswith("local:"):
            _, a, b = spec.split(":", 2)
            stage = StageModule(runtime, sd, int(a), int(b), args.device, args)
...
def cmd_infer(args: argparse.Namespace) -> None:
    ...
    session_id = args.session_id or f"agillm4-{uuid.uuid4().hex}"
...
def cmd_compare(args: argparse.Namespace) -> None:
    ...
    args.device = resolve_device(args.device)
    if bool(sd["cfg"].get("anchor_memory", False)):
        raise SystemExit("distributed compare does not support anchor_memory yet")
    stages = parse_stage_specs(args, runtime, sd)
    emb, ln, ar_h = restore_heads(runtime, sd, args.device)
    cfg = sd["cfg"]
    tie_weights = bool(sd.get("tie_weights", False))
    prompt_tokens = runtime.tok.encode(args.prompt) or [runtime.EOS]
    ids = torch.tensor([prompt_tokens], dtype=torch.long)
    with torch.no_grad():
        hidden = emb(ids.to(args.device)).detach().cpu()
        hidden, stage_stats = run_stage_pipeline(stages, hidden, args, use_cache=False)
        dist_h = ln(hidden.to(args.device))
        dist_logits = ar_h(dist_h)[:, -1].detach().float().cpu()

        core = runtime.Encoder(
            cfg,
            tie_weights=tie_weights,
            **encoder_kwargs(runtime, args),
        ).to(args.device)
        core.load_state_dict(runtime._prepare_core_state_dict_for_load(core, sd["core"]))
        core.eval()
        mask = make_dense_mask("ar", int(ids.size(1)), args.device, args.sat_block)
        mono_h = core(ids.to(args.device), mask)
        mono_logits = ar_h(mono_h)[:, -1].detach().float().cpu()

    diff = (dist_logits - mono_logits).abs()
    result = {
        "event": "agillm4_distributed_compare",
        "prompt_tokens": int(ids.size(1)),
        "max_abs_logit_diff": float(diff.max().item()),
        "mean_abs_logit_diff": float(diff.mean().item()),
        "monolithic_top1": int(mono_logits.argmax(dim=-1).item()),
        "distributed_top1": int(dist_logits.argmax(dim=-1).item()),
        "top1_match": bool(mono_logits.argmax(dim=-1).item() == dist_logits.argmax(dim=-1).item()),
        "stages": stage_stats,
    }
    print(json.dumps(result, indent=2))
...
def main() -> int:
    ap = argparse.ArgumentParser(description="AGILLM4 distributed transformer/MoE/DiffusionBlock inference")
    sub = ap.add_subparsers(dest="cmd", required=True)
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--agillm4-path", default=os.environ.get("AGILLM4_RUNTIME", "./nB300_agillm4.py"))
    common.add_argument("--ckpt", required=True)
    common.add_argument("--attn-backend", choices=["manual", "sdpa", "sublinear"], default=os.environ.get("AGILLM_ATTN_BACKEND", "manual"))
    common.add_argument("--sublinear-window", type=int, default=int(os.environ.get("AGILLM_SUBLINEAR_WINDOW", "256")))
    common.add_argument("--sublinear-stride", type=int, default=int(os.environ.get("AGILLM_SUBLINEAR_STRIDE", "64")))
    common.add_argument("--sublinear-max-anchors", type=int, default=int(os.environ.get("AGILLM_SUBLINEAR_MAX_ANCHORS", "256")))
    common.add_argument("--sublinear-chunk", type=int, default=int(os.environ.get("AGILLM_SUBLINEAR_CHUNK", "128")))
    common.add_argument("--sublinear-sinks", type=int, default=int(os.environ.get("AGILLM_SUBLINEAR_SINKS", "4")))
    common.add_argument("--sublinear-recent-anchors", type=int, default=int(os.environ.get("AGILLM_SUBLINEAR_RECENT_ANCHORS", "-1")))
    common.add_argument("--sublinear-pooled-landmarks", action=argparse.BooleanOptionalAction, default=bool(int(os.environ.get("AGILLM_SUBLINEAR_POOLED_LANDMARKS", "0"))))
    ...
    p.add_argument("--token", default=os.environ.get("AGILLM4_INFER_TOKEN", ""))
